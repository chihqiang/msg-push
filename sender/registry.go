// Package sender 提供消息发送器层：定义 Sender/StatusQuerier/BatchSender 接口、
// 服务商元信息注册表（可用服务商列表与配置字段定义）与各服务商实现（供 worker 消费端调用）。
//
// 服务商元信息当前为静态内置清单，供服务商账号管理使用；实际发送由各 Sender 实现按 provider_code 解析执行。
package sender
